...
func run() error {
	...
	var readerInput io.Reader = f
	buffered := false
	if mode == "unchunked" && filter == "" {
		// Sequential unindexed read: buffer it, matching the Rust bench's
		// BufReader. The indexed modes keep the raw *os.File they need for
		// seeking (and buffer internally per section).
		readerInput = bufio.NewReaderSize(f, 1<<20)
		buffered = true
	}
	reader, err := mcap.NewReader(readerInput)
	if err != nil {
	...
	if buffered {
		// A buffered reader cannot seek; stream in file order without the index.
		opts = append(opts, mcap.UsingIndex(false))
	}
	...
	fmt.Printf("read\tgo\t%s\t%s\t%s\t%d\t%d\t%.6f\t%d\t%d\n",
		mode, numMessagesStr, payloadSizeStr, fileSize, elapsed.Nanoseconds(), elapsed.Seconds(), peakRssKb(),
		msgCount)
