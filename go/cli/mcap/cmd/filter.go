...
usage:
  mcap filter in.mcap -o out.mcap -y /diagnostics -y /tf -y /camera_(front|back)`,
		...
		chunkSize := filterCmd.PersistentFlags().Int64P("chunk-size", "", 0, "chunk size of output file")
...
usage:
  mcap compress in.mcap -o out.mcap`,
		...
		chunkSize := compressCmd.PersistentFlags().Int64P("chunk-size", "", 0, "chunk size of output file")
...
usage:
  mcap decompress in.mcap -o out.mcap`,
		...
		chunkSize := decompressCmd.PersistentFlags().Int64P("chunk-size", "", 0, "chunk size of output file")
