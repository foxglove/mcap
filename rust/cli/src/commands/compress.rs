...
pub async fn execute(args: CompressArgs) -> CliResult<()> {
    ...
    // Process all messages and write them with compression
    ...
    println!("Compressed MCAP file written to: {}", args.output);
    ...
    let compression = match args.compression.as_str() {
        ...
        other => {
            return Err(crate::error::CliError::invalid_argument(format!(
                "Unknown compression format: {}. Use 'lz4' or 'zstd'.",
                other
            )))
        }
    };
