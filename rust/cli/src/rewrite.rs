...
//! read / select / place pipeline and its [`run`] entrypoint), [`merge`] (the multi-input k-way
//! merge pipeline and its [`run_merge`] entrypoint), and [`common`] (the low-level helpers both
//! pipelines share).
mod common;
mod merge;
mod options;
