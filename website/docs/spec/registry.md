...
Note: Arrow is columnar, but MCAP is message-oriented, so multiple rows should be packed into a single `RecordBatch` only when they share the same `log_time`; otherwise each row maps to its own message.
