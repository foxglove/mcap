...
- `name`: The `displayName` of the root type's node: the path passed to `capnp compile` followed by the type's scoped name, e.g. `myschema.capnp:Foo.Bar`. Readers locate the root type by finding the struct node whose `displayName` exactly matches this value, which must identify a single node.
- `encoding`: `capnproto`
