...
- `encoding`: `capnproto`
- `data`: A binary [CodeGeneratorRequest](https://github.com/capnproto/capnproto/blob/master/c%2B%2B/src/capnp/schema.capnp) describing the schema and its imports ([stream framing](https://capnproto.org/encoding.html#serialization-over-a-stream), unpacked), as produced by `capnp compile -o - myschema.capnp`.
