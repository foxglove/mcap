...
- `name`: The `displayName` of the root type's node: the path passed to `capnp compile` followed by the type's scoped name, e.g. `myschema.capnp:Foo.Bar`. Readers locate the root type by finding the struct node whose `displayName` matches this value exactly, which must identify exactly one node.
- `encoding`: `capnproto`
