...
- Drag the divider after the channel/topic column to resize it. Both resize handles support arrow keys (10 px, or 50 px with Shift), Home/End for limits, and Enter or double-click to reset. Resizing preserves the time viewport and does not load more data.
- Message counts include approximate frequency: `(count − 1) / (last log time − first log time)`. Channel rows use their loaded messages, or their chunk’s messages during drill-down; chunk details use the chunk’s full count and time range. Recording stats use the aggregate loaded messages. These are observed averages that may change as data loads; fewer than two messages or equal timestamps show a dash.
