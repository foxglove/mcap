...
- Drag the divider after the channel/topic column to resize it. Both resize handles appear only when a recording is loaded, with no foreground loading or error. The height handle retains its layout space during loading to keep navigation stationary. Both handles support arrow keys (10 px, or 50 px with Shift) and Home/End for limits. Resizing preserves the time viewport and does not load more data.
- Frequencies use `message count / interval duration`, including silent periods, consistent with `mcap info` once the whole recording is loaded. All channel rows use the same loaded interval (or the full recording for a complete snapshot), never each channel's first/last message times. During chunk drill-down, use the intersection of the loaded interval and the chunk; full chunk counts in tooltips/Details use the full chunk duration. The interval for channel rates is displayed above the canvas. Recording stats use the aggregate count over the loaded interval. Frequencies display at most one decimal place, without an approximation symbol (positive rates below 0.05 Hz display as `<0.1 Hz`); fewer than two messages, an unknown interval, or zero duration show a dash. Rates may change as preloading expands coverage; zooming within already loaded data does not change their interval.
